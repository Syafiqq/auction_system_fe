...
import SearchForm from "@/app/profile/auctions/components/search-form";

const AuctionParticipationTable = () => {
            ...
            const response = await userRepository.auctionParticipationFromRemote(
                {
                    ...
                    types: params.getAll('types[]').map((value) => mapTypes(value))
                }
            )
            setData(response);
        } catch (e) {
    ...
    const mapTypes = (value: string): AuctionParticipationRequestStatus => {
        switch (value) {
            case '1':
                return AuctionParticipationRequestStatus.WIN;
            case '2':
                return AuctionParticipationRequestStatus.LOSE;
            case '3':
                return AuctionParticipationRequestStatus.IN_PROGRESS;
            case '4':
                return AuctionParticipationRequestStatus.IN_PROGRESS_LEADING;
            default:
                return AuctionParticipationRequestStatus.WIN;
        }
    };
    ...
    const handleView = (id: number) => {
        router.push(`/auction/${id}/place-bid`);
    };
