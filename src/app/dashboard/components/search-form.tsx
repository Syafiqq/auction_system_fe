...
const SearchForm = ({requestFetchData, queryString}: {
    ...
    // Reverse mapping for initial state
    const reverseSortPriceOptions = {
        'Not sorted': '',
        'Lowest first': 'asc',
        'Highest first': 'desc',
    };
    ...
    const [sortPrice, setSortPrice] = useState(queryString.get('sort_price') || '');
    ...
    useEffect(() => {
        ...
        if (sortPrice) {
            queryString.set('sort_price', sortPrice);
        } else {
            queryString.delete('sort_price');
        }

        router.replace(`?${queryString.toString()}`);
    }, [name, description, sortPrice]);
            ...
            <form className="space-y-4" onSubmit={handleSearch}>
                ...
                <div>
                    ...
                    <select
                        ...
                        className="w-full px-4 py-2 border rounded-md"
                    >
                        {Object.entries(sortPriceOptions).map(([value, label]) => (
